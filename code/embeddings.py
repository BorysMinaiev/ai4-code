...
def get_embedding(nb, cell_id):
    start = time.time()
    cell = nb.loc[cell_id]
    tokens = unixcoder_model.tokenize(
        [cell['source']], max_length=512, mode="<encoder-only>")
...
#     TODO: maybe different size?
    ...
    for nb in np.array_split(nb, n_chunks):
        ...
        tokens = unixcoder_model.tokenize(nb['source'].to_numpy(
        ), max_length=512, mode="<encoder-only>", padding=True)
        source_ids = torch.tensor(tokens).to(device)
        _, embeddings = unixcoder_model(source_ids)
